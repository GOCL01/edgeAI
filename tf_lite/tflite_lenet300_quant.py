...
########################################################################################################
    ...
    Source Name  : tflite_lenet300_quant.py
    Description  : converting dense h5 model of lenet300 to tflite 
...
model = tf.keras.models.load_model('model_dense_lenet300.hdf5')
...
# Save the model
open ("model_q.tflite" , "wb") .write(tflite_quant_model)
interpreter = tf.lite.Interpreter(model_path="model_q.tflite")
...
input_data = x_test[0].reshape(1,784).astype(np.float32)
...
for i in range(0, 9999):
    input_data = x_test[i].reshape(1,784).astype(np.float32)
    interpreter.set_tensor(input_details[0]['index'], input_data)
